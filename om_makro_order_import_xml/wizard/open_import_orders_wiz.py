...
from odoo.exceptions import UserError, w
...
class MakroImportOrdersWizard(models.TransientModel):
    ...
    def _get_product_id(self, product_code, product_name):
        product_ids = self.env['om_makro_order_import_xml.makro_products'].search([('makro_code', '=', product_code)]).product_ids

        if product_ids:
            if len(product_ids) > 1:
                self._auto_select_product_the_mose(product_ids)
                return {'warning': {
                        'title': 'Warning',
                        'message': ("Message"),
                    }}
            ...
            print(product_code, product_name, False)
    ...
    def action_import_orders(self):
        ...
        if file:
            ...
            for node in xroot:
                for po in node.findall('POHEADER'):
                    for line in po.findall('POLINE'):
                        product_id = self._get_product_id(line.find('BUYERARTNO').text, line.find('PRODUCTDESCRIPTION').text)
        #     order_ids = []
